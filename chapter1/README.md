...
读完引言和第一章，最大的感受是：这本书不是在教你"怎么用某个框架跑通一个 Demo"，而是在回答一个更根本的问题——**Agent 为什么这样设计，以及每个设计决策背后的能力边界与工程取舍是什么**。

引言中有一句话让我印象很深："**实践在前，命名在后**。"Skill、Harness、Loop Engineering 这些概念听起来很新，但对应的工程实践其实更早出现在真实的生产系统里。概念是对大量实践经验的归纳，而不是实践的起点。这意味着我们不应该等"学会了某个概念"才开始动手，而应该在做中学、在问题中提炼。

第一章给出了全书最核心的公式：**Agent = LLM + 上下文 + 工具**。用更直觉的方式说，就是 **大脑 + 眼睛 + 手脚**——大脑负责思考和决策，眼睛决定 Agent 能看到什么信息，手脚决定 Agent 能做什么操作。三者缺一不可。这个公式看似简单，但它解释了很多看似复杂的现象：

- **为什么有时候换个模型就能解决问题？** 因为你升级了"大脑"。
- **为什么有时候模型明明很强，任务还是完不成？** 因为"眼睛"看不到关键信息（上下文缺失），或者"手脚"够不到需要的操作（工具缺失）。
- **为什么 Manus 能成为通用 Agent？** 不是因为它换了更强的模型，而是它合并了原本分离的观察空间和动作空间——浏览器扩大了眼睛，文件系统扩大了手脚。

另一个重要概念是 **ReAct 循环**（思考 → 行动 → 观察）。Agent 不是一次性给出答案，而是在循环中不断推进任务：先思考当前该做什么，再调用工具行动，然后观察结果，继续思考下一步。这个循环是几乎所有 Agent 系统的基本运行模式。实验 1-2（Kimi K3 原生 Agent）和实验 1-3（Deep Research）都直观地展示了这个循环的运作。

但一个能跑的 Demo 和一个可靠的产品之间还有巨大的鸿沟——这就是 **Harness 工程**要解决的问题。Harness 是模型之外的运行与治理层，包括上下文管理、工具接口、约束、验证和纠正。书中用了一个很形象的比喻：模型是那匹强大但不可预测的马，Harness 就是把它的能力引导到正确方向上的缰绳。行业正在从"能做事"向"可靠地做事"转变，**当各家模型能力越来越接近时，竞争优势就转移到了模型之外的 Harness 工程上**。

第一章还给出了编排模式的演进路线：**先优化提示词 → 再考虑工作流 → 最后才引入自主 Agent**。不是越复杂越好，而是从简单开始，只在确实必要时才增加复杂度。这与 Anthropic 总结的核心原则一致：**保持简单、保持透明、设计好工具接口**。

最后，引言中 Richard Sutton 的那段话给了我很大的触动：Agent 是宇宙演化史上一种全新的存在——它能理解自身的运作机制，并根据目标创造全新的智能体，甚至改进自己。这本书的使命，就是帮助我们理解和掌握这种创造的原则。

带着这些理解去做后面的实验，会发现每个实验都不只是在"跑通一个功能"，而是在验证和体会这些设计原则。下面五个实验的归档记录，就是我在动手实践中对这些概念的具体印证。
